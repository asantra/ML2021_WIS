...
class EdgeNetwork(nn.Module):
    def __init__(self,inputsize,hidden_layer_size,output_size):
        ...
        self.net = nn.Sequential(
            nn.Linear(inputsize, hidden_layer_size*10),
            nn.ReLU(),
            #nn.Linear(hidden_layer_size*10, hidden_layer_size*5),
            #nn.ReLU(),
            nn.Linear(hidden_layer_size*10, hidden_layer_size),
            nn.ReLU(),
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.node_init = nn.Sequential(
                       nn.Linear(2,2*(node_hidden_dim*4+2)),
                       nn.ReLU(),
                       nn.Linear(2*(node_hidden_dim*4+2),node_hidden_dim*4+2),
                       nn.ReLU(),
                       nn.Linear(node_hidden_dim*4+2, node_hidden_dim))
        
        self.edge_network    = EdgeNetwork(2*(2+node_hidden_dim)+1,2*(edge_hidden_dim*4+2),edge_hidden_dim)
        self.node_network    = NodeNetwork(2+edge_hidden_dim+node_hidden_dim,2*(edge_hidden_dim*4+2),node_hidden_dim)
        self.edge_classifier = EdgeNetwork(2*(2+node_hidden_dim)+1,2*(edge_hidden_dim*4+2),1) # remember the output size should be configurable!
        self.node_classifier = nn.Sequential(
                                 nn.Linear(node_hidden_dim,2*(edge_hidden_dim*4+2)),
                                 nn.ReLU(),
                                 #nn.Linear(2*(edge_hidden_dim*4+2), edge_hidden_dim*4+2),
                                 #nn.ReLU(),
                                 nn.Linear(2*(edge_hidden_dim*4+2), edge_hidden_dim),
                                 nn.ReLU(),
